...
class Supreme extends Task {
	...
	async run() {
		try {
			await this.init();
			if (this.taskData.mode === 'browser-mobile' || 'browser-desktop') await frontend.init.bind(this)();
		...
		return new Promise((resolve, reject) => {
			this.request({
				...
				if (!error && response.statusCode === 200) {
					...
					let styleData;
					if (this.products[0].style === '' || this.products[0].style.trim().toLowerCase() === 'any') {
						styleData = allStyleData[0];
					}
					else {
						for (let i = 0; i < allStyleData.length; i++) {
							...
							if (keyword.isMatch(unidecode(styleName), variantKeywords)) break;
